...
async function updateDocs(sessionId: string): Promise<void> {
  ...
  // Load docs prompt from file
  const promptPath = join(__dirname, '..', 'prompts', 'docs.md');
  const content = await readFile(promptPath, 'utf-8');
  const match = content.match(/```\n([\s\S]*?)```/);
  const promptTemplate = match ? match[1] : content;
...
Take this into account as you proceed with the next iteration.
  ...
  // Load main loop prompt from file
  const promptPath = join(__dirname, '..', 'prompts', 'loop.md');
  const content = await readFile(promptPath, 'utf-8');
  const match = content.match(/```\n([\s\S]*?)```/);
  if (!match) {
    throw new Error('Could not find prompt content in loop.md');
  }
  const promptTemplate = match[1];

  return promptTemplate
    .replace('${process.cwd()}', process.cwd())
    .replace('${sessionContent}', sessionContent)
    .replace('${sessionPath}', sessionPath)
    .replace('${userMessageSection}', userMessageSection);
}
